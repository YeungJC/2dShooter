...
function Director:new(stage)
    ...
    self.resource_spawn_chances = chanceList({'Boost', 28}, {'HealthPack', 14}, {'SP', 58})
    self.weapon_spawn_chances = chanceList({'Boost', 28}, {'HealthPack', 14}, {'SP', 58})

    self.timer:every(16, function() self.stage.area:addGameObject(self.resource_spawn_chances:next()) end)
    self.timer:every(22, function() self.stage.area:addGameObject('AttackBox') end)
end
